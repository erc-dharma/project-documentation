  ...
  <!-- Contains named templates for EDH file structure (aka "metadata" aka "supporting data") -->
   ...
   <xsl:template name="dharma-structure">
      ...
      <html>
         ...
         <body>
            <h1>
              <xsl:value-of select="//t:teiHeader//t:title[@type='main']"/>
              <xsl:text>:&#x202F;</xsl:text>
              <xsl:value-of select="//t:teiHeader//t:title[@type='sub']"/>
            </h1>
        ...
        <i><xsl:value-of select="translate(replace(//t:idno[@type='filename'], 'CritEd', ''), '-', '')"/></i>
        </xsl:element>
      </xsl:if>
      <xsl:if test="//t:teiHeader//t:editor">
        <xsl:element name="p">
      <xsl:text>Editor:&#x202F;</xsl:text>
      <xsl:value-of select="//t:teiHeader//t:editor/t:forename"/>
      <xsl:text> </xsl:text>
      <xsl:value-of select="//t:teiHeader//t:editor/t:surname"/>
      </xsl:element>
      </xsl:if>
           <xsl:if test="//t:teiHeader/t:fileDesc/t:sourceDesc/t:msDesc/t:msContents/t:summary/text()">
           ...
           <xsl:element name="div">
           <xsl:attribute name="id">witnessesdiv</xsl:attribute>
           <h2>Witnesses</h2>
           <xsl:if test="//t:teiHeader/t:fileDesc/t:sourceDesc/t:listWit/t:witness">
             <xsl:for-each select="//t:teiHeader/t:fileDesc/t:sourceDesc/t:listWit/t:witness">
               <p><i><xsl:value-of select="./@xml:id"/></i>
               <xsl:text>:&#x202F;</xsl:text>
...
<!-- Description list-->
       ...
       <xsl:attribute name="id">witnessesdiv</xsl:attribute>
         ...
         <xsl:if test="//t:teiHeader/t:fileDesc/t:sourceDesc/t:listWit/t:witness">
           ...
           <dl>
           <xsl:for-each select="//t:teiHeader/t:fileDesc/t:sourceDesc/t:listWit/t:witness">
             <dt><xsl:value-of select="./@xml:id"/></dt>
