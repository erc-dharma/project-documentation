...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.tei-c.org/ns/1.0"
   ...
   <!--
    ...
    3. Add to -app template below using local-name() to define context
  -->
   ...
   <xsl:template name="dharma-app">
      ...
      <xsl:variable name="childtype">
         <xsl:choose>
            ...
            <xsl:when test="child::t:*[local-name()=('orig','sic','add','lem')]/t:app">
               <xsl:text>app</xsl:text>
            </xsl:when>
            <xsl:when test="child::t:*[local-name()=('note')]/t:app"/>
         </xsl:choose>
      </xsl:variable>
      ...
      <xsl:choose>
         <xsl:when test="not(ancestor::t:choice or ancestor::t:subst or ancestor::t:app)">
         ...
         <xsl:choose>
            <xsl:when test="local-name()=('choice','subst','app')">
              ...
              <xsl:attribute name="title"><xsl:apply-templates select="$title" mode="sqbrackets"/></xsl:attribute>-->
               <!-- <xsl:if test="$apptype = ('appbl')"> and $childtype != ''">  'siccorr', 'subst', 'origreg', 'app' and $childtype='subst'"> -->
   ...
   <xsl:template name="appcontent">
      ...
      <xsl:variable name="path">
         <xsl:choose>
            ...
            <xsl:when test="$childtype='app'">
               <xsl:copy-of select="child::t:*[local-name()=('orig','sic','add','lem')]/t:app/child::*"/>
            </xsl:when>
   ...
   <xsl:template name="appchoice">
     ...
     <!-- {{ (<xsl:value-of select="$path/*" />)
      ...
      <xsl:choose>
         ...
         <xsl:when test="$apptype='app'">
            <!-- **ALT - <xsl:value-of select="$path/t:rdg"/>** -->
            <xsl:for-each select="$path/t:lem">
            <xsl:choose>
            <xsl:when test="@wit">
          ...
          <xsl:text>:&#x202F;</xsl:text>
            ...
            <xsl:value-of select="$path/t:lem"/>
            <xsl:if test="@type">
              <xsl:text>: </xsl:text>
              <xsl:choose>
          ...
          <xsl:if test="$path/t:lem[following-sibling::t:rdg]">
            <xsl:text> ◇ </xsl:text>
          </xsl:if>
        </xsl:for-each>
        ...
        <xsl:choose>
             <xsl:when test="child::t:pb"/>
             <xsl:when test="(not(.//text())) and (not(.//t:gap))">
                <xsl:text>∅ </xsl:text>
             </xsl:when>
             </xsl:choose>
             <xsl:apply-templates/>
             <xsl:if test="following-sibling::t:note and not(following-sibling::t:rdg)">
               <xsl:text> • </xsl:text>
         <xsl:value-of select="following-sibling::t:note"/>
             </xsl:if>
          </xsl:for-each>
          <xsl:if test="not(t:rdg) and t:note">
            <xsl:text> • </xsl:text>
      ...
      <xsl:variable name="no-l">
         <xsl:choose>
            <xsl:when test="$apptype = 'siccorr'"><xsl:value-of select="substring-before(replace($scribeswitch, 'l\. ', 'for which correct to '), '(corr)')"
               /></xsl:when>
            <xsl:when test="$apptype = ('origreg', 'subst', 'app', 'apped')"><xsl:value-of select="replace($scribeswitch, 'l\. ', 'for which read ')"/></xsl:when>
