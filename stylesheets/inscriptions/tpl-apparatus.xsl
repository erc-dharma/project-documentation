...
<!--     
 -->
   ...
   <xsl:template name="campa-app">
      <xsl:for-each select="//t:div[@type='apparatus']"><div class="apparatus">
         <xsl:for-each select="t:p | t:ab">
            <p><xsl:for-each select="t:app">
               <xsl:if test="count(preceding-sibling::t:app) &gt; 0"><xsl:text> — </xsl:text></xsl:if>
               <xsl:choose>
                  <xsl:when test="@n and (@n != preceding-sibling::t:app[1]/@n or not(preceding-sibling::t:app))">
                     <xsl:text></xsl:text><span class="apploc"><xsl:value-of select="@n"/></span><xsl:text> </xsl:text>
                  </xsl:when>
                  <xsl:when test="@loc and (@loc != preceding-sibling::t:app[1]/@loc or not(preceding-sibling::t:app))">
                     <xsl:text></xsl:text><span class="apploc"><xsl:value-of select="@loc"/></span><xsl:text> </xsl:text>
                  </xsl:when>
                  <!-- <xsl:when test="count(preceding-sibling::t:app) = 0">
                     <xsl:text></xsl:text><span class="apploc"><span class="check">##</span></span><xsl:text> </xsl:text>
                  </xsl:when> -->
               </xsl:choose>
               <xsl:for-each select="t:lem">
                  <xsl:text></xsl:text><span class="lem"><xsl:apply-templates /></span><xsl:text></xsl:text>
                  <xsl:if test="following-sibling::t:note or following-sibling::t:rdg or ./@resp">
                     <xsl:text> ◇ </xsl:text>    
                  </xsl:if>
                   <xsl:if test="./@resp">
                       <xsl:text></xsl:text><xsl:value-of select="./@resp"/><xsl:text></xsl:text>
                       <xsl:if test="following-sibling::t:note or following-sibling::t:rdg">
                           <xsl:text>;</xsl:text>
                       </xsl:if>
                   </xsl:if>
               </xsl:for-each>
               <xsl:for-each select="t:rdg | t:note"> 
                  
                  <xsl:text></xsl:text><xsl:if test="preceding-sibling::t:lem or preceding-sibling::t:rdg or preceding-sibling::t:note"><xsl:text> </xsl:text></xsl:if><span class="{local-name()}"><xsl:apply-templates select="./node()"/></span><xsl:if test="@resp"><xsl:text> </xsl:text><span class="rdgresp"><xsl:value-of select="@resp"/></span></xsl:if><xsl:text></xsl:text>
                  <xsl:choose>
                     <xsl:when test="local-name()='note' and not(following-sibling::t:note)">
                        <xsl:variable name="lastnotestring">
                           <xsl:text></xsl:text><xsl:value-of select="normalize-space(./text()[last()])"/><xsl:text></xsl:text>
                        </xsl:variable>
                        <xsl:if test="substring($lastnotestring, string-length($lastnotestring), 1) != '.'"><xsl:text>.</xsl:text></xsl:if>
                        
                     </xsl:when>
                     <xsl:when test="local-name()='rdg' and local-name(following-sibling::*[1])='rdg'">
                        <xsl:text>;</xsl:text>
                     </xsl:when>
                     <xsl:otherwise>
                        <xsl:text>.</xsl:text>
