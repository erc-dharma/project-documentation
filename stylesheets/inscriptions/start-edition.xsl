...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:include href="htm-teipersName.xsl"/>
   <xsl:include href="htm-teiquote.xsl"/>
   <xsl:include href="htm-teidate.xsl"/>
   ...
   <xsl:include href="tpl-text.xsl"/>
    ...
    <xsl:include href="custom-tpl-campa.xsl"/>
   ...
   <xsl:template match="/">
      <xsl:choose>
         ...
         <xsl:when test="$edn-structure='campa'">
            <xsl:call-template name="campa-structure"/>
         </xsl:when>
         <xsl:otherwise>
